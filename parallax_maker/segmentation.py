...
def render_view(
    ...
    is_zero_parallax = (
        np.allclose(pixel_shift_x, 0.0, atol=1e-5)
        and np.allclose(pixel_shift_y, 0.0, atol=1e-5)
        and abs(scaled_dz) < 1e-5
    )

    if is_zero_parallax:
        slices_to_use = original_slices if original_slices is not None else image_slices
        for slice_item in slices_to_use:
            cur_h, cur_w = slice_item.image.shape[:2]
            pad_h = (cur_h - h_orig) // 2
            pad_w = (cur_w - w_orig) // 2
            if original_slices is not None and slice_item.image.shape[0] == h_orig and slice_item.image.shape[1] == w_orig:
                # original slices are unpadded and match exactly
                slice_img = slice_item.image
            else:
                slice_img = slice_item.image[pad_h:pad_h+h_orig, pad_w:pad_w+w_orig]

            alpha_channel = slice_img[:, :, 3].astype(np.float32) / 255.0
            alpha_expanded = np.expand_dims(alpha_channel, axis=2)
            rendered_image[:, :, :3] = ((1.0 - alpha_expanded) * rendered_image[:, :, :3] + alpha_expanded * slice_img[:, :, :3]).astype(np.uint8)
            rendered_image[:, :, 3] = np.maximum(rendered_image[:, :, 3], slice_img[:, :, 3])

        provenance_map = np.where(rendered_image[:, :, 3] > 10, 1, 0).astype(np.uint8)
        disocclusion_mask = np.zeros((h_orig, w_orig), dtype=np.uint8)
        ...
        final_output.original_pixel_percentage = float(np.count_nonzero(provenance_map == 1)) / float(h_orig * w_orig) * 100.0
    ...
    for i in range(len(image_slices)):
        ...
        # Check if layer is static (such as the visual anchor middleground layer)
        scale_factor_i = Z_clamped[i] / (Z_clamped[i] - dz_layers[i]) if (Z_clamped[i] - dz_layers[i]) > 0 else 1.0
        is_layer_static = (
            abs(dx_layers[i]) < 0.05
            and abs(dy_layers[i]) < 0.05
            and abs(scale_factor_i - 1.0) < 1e-4
            and abs(yaw) < 0.01
            and abs(pitch) < 0.01
        )
        ...
        layer_unpadded_active = (warped_orig[:, :, 3] > 10)
        layer_padded_active = (warped_padded[:, :, 3] > 10)

        # Vectorized C-level composition via np.where
